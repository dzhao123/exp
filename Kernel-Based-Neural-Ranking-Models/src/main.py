...
def train(model, opt, crit, optimizer, train_data, dev_data, test_data):
    ...
    for epoch_i in range(opt.epoch):
        ...
        for batch in train_data:
            ...
            crit1 = nn.MarginRankingLoss(margin=0.1, size_average=True)
            crit2 = nn.MarginRankingLoss(margin=0.1, size_average=True)
            ...
            batch_loss_pos = crit1(0.1*outputs_pos_axm, 0.1*outputs_pos_adj, Variable(0.1*label, requires_grad=False))
            batch_loss_neg = crit2(0.1*outputs_neg_axm, 0.1*outputs_neg_adj, Variable(0.1*label, requires_grad=False))
            #print(batch_loss_origin)
            #print(batch_loss_pos)
            #print(batch_loss_neg)
            batch_loss = batch_loss_origin + 0.1*batch_loss_pos + 0.1*batch_loss_neg
            ...
            if step % opt.eval_step == 0:
                ...
                if opt.save_model:
                    ...
                    if opt.save_mode == 'all':
                        model_name = '/work/ececis_research/Manning' + opt.save_model + '_step_{}.chkpt'.format(step)#'../chkpt/' + opt.save_model + '_step_{}.chkpt'.format(step)
                        torch.save(checkpoint, model_name)
                    elif opt.save_mode == 'best':
                        model_name = '/work/ececis_research/Manning/' + opt.save_model + '.chkpt'#'../chkpt/' + opt.save_model + '.chkpt'
                        if map_dev == best_map_dev:
                            best_map_dev = map_dev
                            best_map_test = map_test
                            best_mrr_dev = mrr_dev
                            best_mrr_test = mrr_test
                            with open('/work/ececis_research/Manning/'+opt.task+".txt",'a') as logf:# record log
